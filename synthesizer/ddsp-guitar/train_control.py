...
from train_defaults import build_control_config, apply_control_overrides, parse_control_args
...
def _build_trainer(config, logger, checkpoint_callback):
    trainer_kwargs = dict(
        logger=logger,
    ...
    if config["gpu"] is None:
        ...
        trainer_kwargs.update(accelerator='gpu', devices=[config["gpu"]])
    return pl.Trainer(**trainer_kwargs)


if __name__ == "__main__":
    cli_args = parse_control_args()
    wandb_config = apply_control_overrides(build_control_config(N_PITCHES, N_BINS_PER_PITCH), cli_args)

    if not wandb_config['synthesis_model_checkpoint'] and not wandb_config.get('with_z', False):
        raise ValueError('synthesis_model_checkpoint is required for control-model training unless with_z=true.')
    ...
    wandb.init(
        ...
        name=wandb_config.get('wandb_run_name'),
        config=wandb_config,
        notes=wandb_config.get('description'),
    )
    ...
    if not wandb_config["overfitting_test"]:
        trn_ds = data.load_prepared_data(
            prepared_data_path=wandb.config.train_prepared_data_path,
            seconds_per_clip=wandb.config.n_seconds,
            sample_rate=wandb.config.model_sample_rate,
            feature_frame_rate=wandb.config.model_ft_frame_rate,
            use_random_offset=True,
            pitch_median_filter_window_size=wandb.config.pitch_median_filter_window_size,
        )
    else:
        trn_ds = data.load_prepared_data(
            prepared_data_path=wandb.config.val_prepared_data_path,
            seconds_per_clip=wandb.config.n_seconds,
            sample_rate=wandb.config.model_sample_rate,
            feature_frame_rate=wandb.config.model_ft_frame_rate,
            use_random_offset=True,
    ...
    val_ds = data.load_prepared_data(
        prepared_data_path=wandb.config.val_prepared_data_path,
        seconds_per_clip=wandb.config.n_seconds,
        sample_rate=wandb.config.model_sample_rate,
        feature_frame_rate=wandb.config.model_ft_frame_rate,
    ...
    wandb_logger = WandbLogger(
        project=wandb.config.wandb_project,
        name=wandb.config.wandb_run_name,
        log_model=True,
        notes=wandb.config.description,
    )
    wandb.run.log_code(".")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=wandb.config.checkpoint_dir,
        filename=f"{wandb_logger.experiment.name}" + "{epoch}",
        save_top_k=5,
        mode='min',
    )

    trainer = _build_trainer(wandb.config, wandb_logger, checkpoint_callback)
    print("experiment name:", wandb_logger.experiment.name)

    trainer.fit(model, trn_dl, val_dl, ckpt_path=wandb.config.resume_checkpoint_path)
