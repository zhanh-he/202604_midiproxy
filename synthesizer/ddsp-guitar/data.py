...
class GuitarSetDataset(torch.utils.data.Dataset):
    ...
    def reindex_data(self, clip_duration, samples_per_clip="n_seconds"):
        self.requested_clip_duration = float(clip_duration)
        self.sample_index_to_data_index = {}
        self.sample_index_to_clip_offset_seconds = {}
        sample_index = 0
        for i in range(len(self.data)):
            if samples_per_clip != "n_seconds":
                raise ValueError(f"Unsupported samples_per_clip={samples_per_clip}")
            clip_seconds = float(self.data[i]["clip_seconds"])
            if clip_seconds + 1e-6 < self.requested_clip_duration:
                continue
            max_start = max(0, int(math.floor(clip_seconds - self.requested_clip_duration)))
            for j in range(max_start + 1):
                self.sample_index_to_data_index[sample_index] = i
                self.sample_index_to_clip_offset_seconds[sample_index] = float(j)
                sample_index += 1
    ...
    def __getitem__(self, sample_idx):
        ...
        if self.requested_clip_duration == "full":
            ...
            if self.use_random_offset:
                random_offset_seconds = float(np.random.random())
                max_offset = max(0.0, clip_seconds - self.requested_clip_duration - start_seconds)
                start_seconds += min(random_offset_seconds, max_offset)
