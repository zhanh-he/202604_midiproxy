...
def derive_chunk_hop(chunk_size, overlap=0.0):
    """
    Convert an overlap ratio into an integer hop size.
    """
    overlap = float(overlap)
    if overlap < 0.0 or overlap >= 1.0:
        raise ValueError("overlap must be in [0, 1)")
    hop_size = int(round(float(chunk_size) * (1.0 - overlap)))
    return max(1, hop_size)

def chunk_audio_and_annotation(audio, f0data, item_dur=ITEM_DUR, sr=SR, frame_rate=FRAME_RATE, overlap=0.0, omit_incomplete_chunks=OMIT_INCOMPLETE_CHUNKS):
    """
    ...
    audio_hop_size = derive_chunk_hop(audio_chunk_size, overlap=overlap)
    anno_hop_size = derive_chunk_hop(anno_chunk_size, overlap=overlap)
    audio_arr = chunk_arr(audio, audio_chunk_size, omit_incomplete_chunks=omit_incomplete_chunks, hop_size=audio_hop_size)

    frequencies = f0data.frequencies
    voicing = f0data.voicing
    freq_arr = chunk_arr(frequencies, anno_chunk_size, omit_incomplete_chunks=omit_incomplete_chunks, hop_size=anno_hop_size)
    voicing_arr = chunk_arr(voicing, anno_chunk_size, omit_incomplete_chunks=omit_incomplete_chunks, hop_size=anno_hop_size)

    return audio_arr, freq_arr, voicing_arr

def chunk_audio(audio, item_dur=ITEM_DUR, sr=SR, overlap=0.0, omit_incomplete_chunks=OMIT_INCOMPLETE_CHUNKS):
    """
...
def chunk_frame_labels(labels, item_dur=ITEM_DUR, frame_rate=FRAME_RATE, overlap=0.0, omit_incomplete_chunks=OMIT_INCOMPLETE_CHUNKS):
    """
    ...
    label_hop_size = derive_chunk_hop(label_chunk_size, overlap=overlap)
    label_arr = chunk_arr(labels, label_chunk_size, omit_incomplete_chunks=omit_incomplete_chunks, hop_size=label_hop_size)
