...
class GuitarSetLoader:
	...
	def get_audio_and_MIDI_conditioning_from_track(self, track_id, sr=SR, frame_rate=FRAME_RATE, item_dur=ITEM_DUR, overlap=0.0, omit_incomplete_chunks=OMIT_INCOMPLETE_CHUNKS):
		"""
		...
		frame_rate : float
			...
			overlap : float
				overlap ratio between adjacent chunks in [0, 1)
		...
		# chunk audio
		mix_audio_arr = gset_proc.chunk_audio(
			mix_audio,
			item_dur=item_dur,
			sr=sr,
			overlap=overlap,
			omit_incomplete_chunks=omit_incomplete_chunks,
		)
		mic_audio_arr = gset_proc.chunk_audio(
			mic_audio,
			item_dur=item_dur,
			sr=sr,
			overlap=overlap,
		...
		track_items = int(min(mix_audio_arr.shape[0], mic_audio_arr.shape[0]))
		mix_audio_arr = mix_audio_arr[:track_items]
		mic_audio_arr = mic_audio_arr[:track_items]
		frames_per_item = int(round( item_dur * frame_rate ))
		frame_hop = gset_proc.derive_chunk_hop(frames_per_item, overlap=overlap)
		frames_per_track = 0 if track_items == 0 else frame_hop * (track_items - 1) + frames_per_item
		...
		for string_idx, string_let in enumerate(GUITAR_STRING_LETTERS):
			...
			midi_pitch_arr = gset_proc.chunk_frame_labels(
				midi_pitches,
				item_dur=item_dur,
			...
			onset_vel = np.zeros((frames_per_track)) # shape: (total frames)
			...
			# chunk onset_vel
			onset_vel_arr = gset_proc.chunk_frame_labels(
				onset_vel,
				item_dur=item_dur,
				frame_rate=frame_rate,
				overlap=overlap,
	...
	def create_midi_conditioning_dataset(self, track_ids, sr=SR, frame_rate=FRAME_RATE, item_dur=ITEM_DUR, overlap=0.0):
		"""
		...
		print("Extracting track data...")
		for id in tqdm_notebook(track_ids):
			track_data = self.get_audio_and_MIDI_conditioning_from_track(
				id,
				sr=sr,
				frame_rate=frame_rate,
				item_dur=item_dur,
	...
	def create_and_save_midi_conditioning_datasets_from_splits(self, splits, save_path=DATASETS_PATH, sr=SR, frame_rate=FRAME_RATE, item_dur=ITEM_DUR, overlap=0.0):
		"""
		...
		sr : float
	        ...
	        """	
		for split_name, track_list in tqdm_notebook(splits.items()):
			dataset = self.create_midi_conditioning_dataset(track_list, sr=sr, frame_rate=frame_rate, item_dur=item_dur, overlap=overlap)
