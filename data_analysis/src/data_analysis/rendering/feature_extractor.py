...
try:
    import torchaudio
    _TORCHAUDIO_ERROR = None
except Exception as exc:  # noqa: BLE001
    torchaudio = None  # type: ignore[assignment]
    _TORCHAUDIO_ERROR = exc
...
def _require_torchaudio() -> None:
    if torchaudio is None:
        raise ImportError(
            "torchaudio is required for log-mel extraction in data_analysis.rendering.feature_extractor. "
            "Install torchaudio or use the bark/sone/ntot extractors instead."
        ) from _TORCHAUDIO_ERROR
...
def get_feature_extractor_and_bins(audio_feature, sample_rate, fft_size, frames_per_second):
    # Support explicit mel-bin variants. Keep "logmel" as alias of 128 for backward compatibility.
    if audio_feature in ("logmel", "logmel128"):
        feature_extractor = LogMelExtractor(sample_rate=sample_rate, fft_size=fft_size,
                                            frames_per_second=frames_per_second, mel_bins=128)
        freq_bins = feature_extractor.mel_bins
    elif audio_feature == "logmel229":
        feature_extractor = LogMelExtractor(sample_rate=sample_rate, fft_size=fft_size,
                                            frames_per_second=frames_per_second, mel_bins=229)
        freq_bins = feature_extractor.mel_bins
...
class LogMelExtractor(nn.Module):
    ...
    """
    def __init__(self, sample_rate=44100, fft_size=1024, frames_per_second=86, mel_bins=128):
        super().__init__()
        _require_torchaudio()
        # 128 default in librosa & torchaudio & Beat This; allow 229 as requested
...
# Reusable helpers for quick CSV/PNG exports.
...
def build_argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Extract Bark or log-Mel features from waveform .h5 files."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sone",
        choices=["sone", "bark", "ntot", "logmel", "logmel128", "logmel229"],
        help="Feature to extract.",
    )
    parser.add_argument("--sample_rate", type=int, default=44100, help="Waveform sample rate")
    parser.add_argument("--fft_size", type=int, default=1024, help="FFT size")
    parser.add_argument("--frames_per_second", type=float, default=86, help="Frames per second")
    parser.add_argument("--plot_path", type=str, default=None, help="Optional PNG path")
    return parser
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.mode in {"logmel", "logmel128", "logmel229"}:
        mel_bins = 229 if args.mode == "logmel229" else 128
        ...
        extracted = extractor(wav_tensor).squeeze(0).numpy()
        features = extracted.T if args.mode != "ntot" else extracted
        hop_duration = extractor.hop_size / extractor.sample_rate
        times = np.arange(features.shape[0]) * hop_duration
        columns = (
            [f"{args.mode}_{i + 1}" for i in range(features.shape[1])]
            if args.mode != "ntot"
            else [args.mode]
        )
